...
<p>
  ...
  <img alt="checks" src="https://img.shields.io/badge/checks-207%20assertions-2ead33">
</p>
...
- **Right now.** A short panel above the list, answering whether this is a good moment to listen. Whether it is dark and how long until that changes, computed on the device — below 30 MHz darkness decides what you can hear, which is why so many entries in the data say "at night". The planetary K index and solar flux, for the same reason. And in the United States, any active National Weather Service alert, with the local NOAA weather frequency next to it; tap the card and the list jumps to it.
  ...
  Once you share your location the panel gains two more cards, **Overhead** and **Line of sight**, each carrying its own one-line answer — the next satellite pass and how long until it, how many of the region's transmitter sites are within reach. Both open into a full panel when tapped. They sit here because they answer the same question as the readings beside them, and they stay closed because they are the two tallest things the site draws and neither is wanted on every visit.
...
- **Line of sight** (Pro) — opened from its card in the Right now panel, in one of three views. **Reach** is a map of the region's transmitter sites with each one's radio horizon drawn around it, so you can see which are geometrically in reach of a handheld and which are behind the curve of the Earth. The rings are the 4/3-refraction horizon for each site's own field elevation plus your 2 m, verified in the suite against the closed-form tangent-to-a-sphere geometry it approximates.

  **Distance** joins you to every site instead, each labelled with the straight-line kilometres, and keeps the in-reach ones marked so a short line to somewhere behind the horizon is not mistaken for a usable one. **Land** puts a coastline and administrative borders under the whole thing, so you can see where you are in ordinary terms.

  The map is drawn from the coordinates in the repository — no tiles, no map library, no network. The outline is real survey data rather than an invented shoreline: `scripts/import-geo.mjs` clips Natural Earth's 1:10m coast and admin-1 lines down to the frame each region's map can actually draw, which is about 10 kB per region, and commits it. It is fetched the first time you choose that view and kept from then on; offline with nothing cached the map says so and draws what it always drew. Satellite imagery and street maps are not offered at all, because both are tile servers and would be blank in exactly the conditions this site is built for. The horizon is still all the map claims: terrain is not modelled, and the panel says so, since the hill in front of you beats any of this arithmetic.
...
- **Search** shares the region chip row rather than taking a band of its own: the magnifier opens a field in place of the chips and closes back to them, so a question most visits never ask costs nothing until it is asked.
- Keyboard: `/` opens search, `Esc` closes it or the country menu, `P` switches mode.
